---
name: gpt-doc
description: V4 Client API 使用指南。1Campus GPT V4 API 的第三方開發者文件，涵蓋 Request/Response 規格、SSE 串流事件、Client Function Call、Context 模板、錯誤處理、Skirk 嵌入整合。當討論 V4 API 呼叫方式、串流實作、事件處理、前端整合、API 請求格式、error codes、tool_outputs、二階段 API、EventSource、Skirk、認證模式（auth mode）、嵌入 AI 助理時使用此 Skill。與 gpt-v4-arch（內部架構）互補，本 Skill 專注於外部 Client 端使用方式。
---
...
| credential 怎麼傳？credentialRouting？ | `references/credential-routing.md` |
...
- `references/credential-routing.md` - Credential 透傳機制（caller credential → function header/query/body）
...
### 可執行範例
- `examples/README.md` - 範例總覽
- `examples/credential-passthrough.md` - 兩個範例如何透過 credential 機制把 access token 帶到 function endpoint
- `examples/v4-streaming/` - V4 二階段 API + EventSource 串流（Express + 純 HTML）
- `examples/skirk-embed/` - Skirk code_exchange + iframe 嵌入
