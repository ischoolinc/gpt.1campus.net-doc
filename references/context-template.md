...
系統會自動注入以下變數到 `context`，所有 preset 都可直接使用，無需呼叫方傳入：

| 變數 | 說明 | 範例值 |
|------|------|--------|
| `ctx.current_date` | 當前日期（台灣時區 `Asia/Taipei`，ISO 格式 `YYYY-MM-DD`） | `2026-04-20` |

### 使用範例

**Preset 中的 instructions：**
```
你是行政助理，今天日期是 <%= ctx.current_date %>（對話記錄可能橫跨數日，回答時請以此為準）。
```

### 注意事項

- **可被覆蓋**：如果呼叫方在 `context` 中自行傳入 `current_date`，會以呼叫方的值為準
- **永遠存在**：即使呼叫方沒傳 `context`，系統也會自動建立並注入內建變數
- **時區固定**：目前僅支援台灣時區，若需其他時區請自行傳入 `context.current_date` 覆蓋
